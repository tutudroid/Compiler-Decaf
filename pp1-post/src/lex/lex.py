...
def lexical_analysis_line(line, line_num, offset):
	...
	for pos in range(len(line)):
		...
		if character in tokens.ALPHABET and word.isdigit() and is_string == False:
			lexical_analysis_output1(word, line_num, pos+offset, -1)
			word = ''
		...
		if (character in tokens.DELIMITER and is_string == False):
			...
			if len(word) != 0 and character not in  '.+_':
				if word[-1] not in tokens.SPACE and word[-1] not in '<=>':
					lexical_analysis_output1(word, line_num, pos+offset, T_DoubleConstant if is_double else -1)
					word = ''
				is_new_word = True
								
			if character in tokens.BAD_DELIMITER:
				lexical_analysis_error_output1("\n*** Error line {0}.\n*** Unrecognized char: '{1}'\n\n".format(line_num, character))
				is_new_word = True
				...
				if pos + 1 < len(line) and line[pos] in '<=>' and  line[pos+1] == '=':
					...
					lexical_analysis_output1(word+character, line_num, pos+1+offset, -1)
					is_new_word = True
				# analyze double. 1.12 etc.	
					...
					if len(word)>0 and  word.isdigit() == True:
						...
						lexical_analysis_output1(word, line_num, pos+offset, -1)						
						lexical_analysis_output1(character, line_num, pos+1+offset, -1)
						is_new_word = True
				# analyze double. 1.12E+2
					...
					if is_double != True:	
						...
						lexical_analysis_output1(word[:len(word)-1], line_num, pos-1+offset, T_DoubleConstant)
						lexical_analysis_output1('E', line_num, pos, -1)
						lexical_analysis_output1(character, line_num, pos+1+offset, -1)
						is_new_word = True
						is_double = False
